    ...
    class TreeNode
    {
        public:
            TreeNode(const QList<QVariant>& data,
                     TreeNode* parent = nullptr); // 数据，父节点
            ~TreeNode();
            ...
            void resetOptionChilds(
                const QString& option,
                QList<TreeNode*>
                    childs); // 重置，把option对应的孩子节点列表设置为childs
            void addOptionChild(
                QString option,
                TreeNode* child); // 在option对应的孩子节点的列表最后，加上child
        ...
        private:
            ...
            QList<QString>
                options; // 选项------特殊情况：只有options，但是每种option没有对应的孩子节点，只是单纯的做选择
            QHash<QString, QList<TreeNode*>>
                option_child_nodes; // 每个选项的孩子列表

            bool is_para;
            bool is_topo;
            ParaTreeViewController*
                para_tree_view_controller; // 当is_topo为真时，存相应的右下方的树状模型
    };
