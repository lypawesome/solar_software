...
TreeNode::TreeNode(const QList<QVariant> &data, TreeNode *parent)  //数据，父节点
{
    parent_node = parent;
    node_data = data;
...
{
    return parent_node == nullptr ? 0 : parent_node->child_nodes.indexOf(const_cast<TreeNode*>(this));
...
bool TreeNode::setData(const QVariant& data, int column)    //设置该节点的第column个属性
{
    if(column<0 || column >= node_data.size())
        return false;
...
void TreeNode::setEditable()
{
    editable = true;
}
void TreeNode::setOptional()
{
    optional = true;
}
bool TreeNode::getEditable()
{
    return editable;
}
bool TreeNode::getOptional()
{
    return optional;
}
QList<QString> TreeNode::getOptions()
{
    return options;
}
void TreeNode::insertOption(QString option)
{
...
void TreeNode::makeChoice(const QString& option )    //做出选择，把mNodeData[1]的数据改了，同时把mChildNodes给改了
{
    if(!optional) return;
    if(option == data(1)) return ;

    //deletaAllChild();
    setData(option,1);
    child_nodes = option_child_nodes.value(option);
}
void TreeNode::resetOptionChilds(const QString& option, QList<TreeNode*>childs)
{
    if(option_child_nodes.contains(option))   //如果已经记录了这一选项，就删掉
    {
        option_child_nodes.remove(option);
    }
    option_child_nodes.insert(option,childs);
    for(auto & item: childs)            //孩子的父节点可以现在就设置，但是父亲中的孩子列表，要在选中这一选项后再设置
        item->setParent(this);
}
