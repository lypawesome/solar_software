...
namespace solar
{
    ParaTreeViewController::ParaTreeViewController(int model_type, Para* object,
                                                   QObject* parent)
        : QObject(parent), tree_model(new TreeModel(model_type))
    {

        tree_model->buildSunTreeModel(dynamic_cast<Sun*>(object));
    }
    ParaTreeViewController::~ParaTreeViewController() { delete tree_model; }

    QAbstractItemModel* ParaTreeViewController::getTreeModel()
    {
        return tree_model;
    }
    void ParaTreeViewController::updateNodeData(const QModelIndex& index,
                                                int column,
