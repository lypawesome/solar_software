...
TreeNode* TreeModel::createNode(QVariant data1,QVariant data2, TreeNode* parent,
                                bool editable, bool optional, bool isOptionChild, QString option )
{
    QList<QVariant> list;
    list.append(data1);
    list.append(data2);
    TreeNode* childNode = new TreeNode(list,parent);
    if(editable) childNode->setEditable();
    if(optional) childNode->setOptional();
    if(parent&&!isOptionChild) parent->appendChild(childNode);
    if(parent&&isOptionChild) parent->addOptionChild(option,childNode);
    return childNode;
}
...
void TreeModel::appendChild(const QModelIndex& index, QList<TreeNode*> childNodes, int count)
{
    TreeNode* clickNode = static_cast<TreeNode*>(index.internalPointer());
    if (clickNode->childCount() && count == 0) {	//刷新时先删除所有子节点
...
QString TreeModel::getNodeColumn(const QModelIndex& index, int column)          //获取index这一节点第column列的数据
{
    if( !index.isValid() ) return "";
...
{
    if(!index.isValid()) return QList<QString>();
...
void TreeModel::makeChoice(const QModelIndex& index, QString option)
{
    ...
    //ParaTreeNode* lastchild = currentNode->child(currentNode->childCount()-1);
...
void TreeModel::buildSunTreeModel(Sun* sun)       //建立一个sun的参数设置treeview
{
    ...
    */
    ...
    createNode("azimuth",sun->getPara("azimuth"),positionNode,true,false);
    createNode("elevation",sun->getPara("elevation"),positionNode,true,false);
    createNode("irradiance",sun->getPara("irradiance"),positionNode,true,false);
    TreeNode* trackable = createNode("trackable",sun->getPara("trackable"),positionNode,false,true);        //这里应该改为可选的
    trackable->insertOption("true");
    trackable->insertOption("false");

    //shape
    TreeNode* shapeNode = createNode("shape","Pillbox",root_node,false,true);
    createNode("thetaMax","1",shapeNode,true,false,true,"Pillbox");    //这里如何设置list的第二个
