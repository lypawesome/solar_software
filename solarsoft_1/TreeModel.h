...
enum treeModelType{
    ...
    LOCATION, SUN, AIR, TERRAIN, CAMERA, CLOUD, //右下角，worldPara设置部分
    NODE,SHAPE,TRACKER,ARRAY    //右下角，scenePara设置部分
};
...
public:
    ...
    TreeNode* createNode(QVariant data1,QVariant data2, TreeNode* parent,
                         bool editable=false, bool optional=false,
                         bool isOptionChild = false, QString option="");  //建立一个新节点
    ...
    void appendChild(const QModelIndex& index, QList<TreeNode*> childNodes, int count); //添加一系列孩子
    void setNodeColumn(QModelIndex index, int column, QVariant value);    //设置index这一节点的第column个属性数据为value
    QString getNodeColumn(const QModelIndex& index, int column);          //获取index这一节点第column列的数据
